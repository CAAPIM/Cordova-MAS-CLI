...
var __masPrepare = {
    ...
    'prepare': function (template, options) {
        ...
        shell.exec(command, function (code, stdout, stderr) {
            ...
            (!options.path || !fs.existsSync(options.path)) ?
                command = 'cp -R ' +
                shell.pwd() + '/plugins/cordova-plugin-mas-' + template + '/sample/html/* ' + shell.pwd() + '/www/' :
                command = 'cp -R ' + options.path + (fs.lstatSync(options.path).isDirectory()?'* ':' ')+ shell.pwd() + '/www/';
            ...
            shell.exec(command, function (code, stdout, stderr) {
                ...
                shell.exec(command, function (code, stdout, stderr) {
                    ...
                    // Completed !!
                    var sampleHTML = options.path ? resolve(options.path) + '/index.html' : shell.pwd() + '/plugins/cordova-plugin-mas-' + template + '/sample/html/index.html';
